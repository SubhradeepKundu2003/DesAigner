...
**Image-rich output DONE (2026-07-10, user request "use default images in
different places + generate a test docx with images"):**
- **Default photos everywhere**: 10 abstract brand-colored placeholder
  PNGs (900×600, gradients + soft shapes, System.Drawing-generated) — one
  per `storage/assets/<SECTION_NAME>/default-photo.png` + an upgraded
  `GENERIC/team-photo.png`. Drop-in replaceable with real photography,
  zero code.
- **`samples/TD_Sample_WithImages.docx`** (83KB, 5 sections, 3 embedded
  photos) generated by `SampleDocGeneratorIT` (manual-run IT, regenerate
  with `-Dtest=SampleDocGeneratorIT`). Exercises the source-document-image
  path end-to-end.
- **Photo slots — layout now RESERVES image space** (the old
  gap-scavenging placed only 1 image on the dense editorial layout):
  `LayoutEngine.reservePhotoSlot` places an empty `IMAGE_PLACEHOLDER` box
  (assetId null, carries the article `SourceLink`) below each STANDARD/
  STAT_CALLOUT/TWO_COLUMN section when the template has photo decor
  (HERO/EVENT_LIST excluded); slot = 150pt tall, 62% content width.
  `ImagePlacer.fillSlots` resolves each (source-doc image first → section
  brand default → GENERIC), applies the full photo treatment, or REMOVES
  the slot (no dashed placeholders in exports). Gap-scavenging remains as
  pass 2 for sections without a slot image.
- **Two defects found on the first e2e and fixed:** (1) a slot that
  didn't fit spilled onto an orphan image-only page → `Paginator.fits()`
  guard, slot skipped instead; (2) duplicate image in one section when
  its slot sat on a later page than its first article → the two passes
  now run page-set-wide sequentially (all slots fill and register their
  section before any scavenging).
- Verified live e2e: the sample docx → 3-page issue, **4 images, one per
  section** — Delivery Highlights/Project Updates/Customer Success from
  the docx's own embedded photos (document-level matching landed them
  correctly), Training & Learning from its brand default; all rounded +
